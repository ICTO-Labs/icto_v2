    ...
    <Teleport to="body">
        ...
        <div v-if="isOpen" class="fixed inset-0 z-60">
            ...
            <Transition name="slide-right" mode="out-in">
                <div class="fixed inset-0 overflow-y-auto" @click="handleClose">
                    <div class="flex min-h-full items-end justify-end transform transition-transform duration-300">
                        <div 
                            v-if="isOpen" 
                            class="w-full sm:w-[600px] bg-white dark:bg-gray-800 shadow-xl flex flex-col min-h-screen transform transition-transform duration-300"
                            :class="{
                                ...
                                <div class="p-4 space-y-3">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <button 
                                                ...
                                                <RefreshCcwIcon :class="{ 'animate-spin': isRefreshing }" class="w-3.5 h-3.5" /> Refresh balance
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div v-for="token in tokens" :key="token.symbol"
                                        ...
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-full bg-gray-100 p-1">
                                                <img :src="token.logoUrl" :alt="token.name" class="w-full h-full object-cover rounded-full" v-if="token?.logoUrl" />
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-600 dark:text-gray-400 font-medium">
                                                    {{ maskedValue(formatBalance(token.balance ?? 0, token.decimals)) }}
                                                    {{ token.symbol }}
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="text-right group-hover:hidden">
                                            <p class="text-sm font-normal dark:text-white">${{ maskedValue(formatToNonZeroDecimal(token?.metrics?.price ?? 0)) }}</p>
                                            <p class="text-sm font-normal dark:text-white text-green-600">
                                                +0.00%
                                            </p>
                                        </div>
...
const { isOpen, totalValue, totalAssets, closeAssets, reloadAllAssets } = useAssets()
...
const handleRemoveToken = async (token: any) => {
    ...
    useSwal.fire({
        title: 'Are you sure?',
        text: 'Are you sure you want to remove this token? You can always add it back later!',
        icon: 'warning',
...
const handleEscKey = (event: KeyboardEvent) => {
    if (event.key === 'Escape' && isOpen.value && !hasOpenChildModal.value) {
        closeAssets()
    }
}
