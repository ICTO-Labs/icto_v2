                  ...
                  <div class="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                    <div
                      ...
                      :class="[
                        ...
                        step.color === 'indigo' ? 'bg-indigo-500' : step.color === 'orange' ? 'bg-orange-500' : 'bg-[#d8a735]'
                      ]"
                    ></div>
                  </div>
...
import type { DistributionTimelineStep } from '@/composables/distribution/useDistributionStatus'
import CountdownTimeline from '@/components/launchpad/CountdownTimeline.vue'
import {
  ClipboardListIcon,
  PlusCircleIcon,
  UsersIcon,
...
const { timeline } = useDistributionStatus(distributionRef)

// Helper to check if timestamp is in future
const isInFuture = (timestamp: bigint | Date | string): boolean => {
  try {
    let timeMs: number
...
const getStepIcon = (step: DistributionTimelineStep) => {
  ...
  const iconMap: Record<string, any> = {
    ...
    'distribution-start': PlayIcon,
    'vesting-period': step.color === 'orange' ? LockIcon : ClockIcon,
    'completed': CheckCircleIcon
  }
  return iconMap[step.id] || CheckCircleIcon
}

// Helper functions for styling
const getStepIconClass = (step: DistributionTimelineStep): string => {
...
const getStepTextClass = (step: DistributionTimelineStep): string => {
  // Next step - Gold text
  if (shouldShowCountdown(step)) {
...
const getConnectorClass = (step: DistributionTimelineStep): string => {
...
const getValueClass = (step: DistributionTimelineStep): string => {
  if (step.status === 'completed') {
...
const formatDateCompact = (timestamp: bigint | Date | string): string => {
  try {
    let date: Date
