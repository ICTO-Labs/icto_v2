<template>
  <AdminLayout>
    <div class="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800">
      ...
      <!-- Main Content -->
      <div v-else-if="launchpad" class="mx-auto px-4">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden mb-6 border border-gray-200 dark:border-gray-700">
          ...
          <div
            class="relative bg-gradient-to-r from-[#d8a735] via-[#eacf6f] to-[#e1b74c] p-6"
          ...
          >
            ...
            <div class="relative flex items-start justify-between">
              <div class="flex items-center space-x-4">
                <div class="w-16 h-16 bg-white rounded-xl shadow-sm flex items-center justify-center overflow-hidden border-2 border-white/30">
                  <img
                    v-if="projectLogo"
                    :src="projectLogo"
                    :alt="projectName"
                ...
                <div>
                  ...
                  <div class="flex items-center space-x-3">
                    ...
                    <span v-if="categoryDisplay" class="px-2 py-0.5 bg-white/20 text-white rounded-full text-xs font-medium">
                      {{ categoryDisplay }}
                    </span>
                  </div>
                </div>
              ...
              <div class="text-right text-white">
                ...
                <p class="text-2xl font-bold">{{ formatAmount(totalRaised, purchaseTokenDecimals) }} {{ purchaseTokenSymbol }}</p>
                <p class="text-xs opacity-80">of {{ formatAmount(hardCap, purchaseTokenDecimals) }} {{ purchaseTokenSymbol }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="px-6 py-4 bg-gray-50 dark:bg-gray-900">
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="text-xs font-semibold text-[#d8a735]">{{ progressPercentage.toFixed(2) }}%</span>
            </div>
            ...
            <div class="flex items-center justify-between mt-2 text-xs text-gray-600 dark:text-gray-400">
              <div>
                <span class="font-medium">Soft Cap:</span> {{ formatAmount(softCap, purchaseTokenDecimals) }} {{ purchaseTokenSymbol }}
                <span v-if="softCapProgress >= 100" class="ml-2 text-green-600 dark:text-green-400">✓</span>
              </div>
              <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2">
            ...
            <div v-show="activeTab === 'overview'" class="space-y-4">
              ...
              <div>
                ...
                <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap">{{ projectDescription }}</p>

                <!-- Project Links -->
                <div v-if="hasProjectLinks" class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">Official Links</h4>
                  <div class="flex flex-wrap gap-2">
                    ...
                    <a
                      v-if="projectTelegram"
                      :href="projectTelegram"
                      target="_blank"
                      class="inline-flex items-center px-3 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-[#eacf6f] hover:text-[#b27c10] dark:hover:bg-[#d8a735] dark:hover:text-white transition-all text-sm"
                    >
                      <svg class="w-3.5 h-3.5 mr-1.5" fill="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Trust Indicators -->
                <div v-if="isAudited || isKYCed" class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  <div class="flex flex-wrap gap-2">
                    <div v-if="isAudited" class="flex items-center px-3 py-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg text-sm">
                      <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      <span class="font-semibold">Audited</span>
                    </div>
                    <div v-if="isKYCed" class="flex items-center px-3 py-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-lg text-sm">
                      <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                      </svg>
                      <span class="font-semibold">KYC Verified</span>
                      <span v-if="kycProvider" class="ml-1.5 text-xs">by {{ kycProvider }}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  <InfoCard label="Token Price" :value="`1 ${saleTokenSymbol} = ${tokenPrice} ${purchaseTokenSymbol}`" />
                  <InfoCard label="Min Contribution" :value="`${formatAmount(minContribution, purchaseTokenDecimals)} ${purchaseTokenSymbol}`" />
                  <InfoCard label="Max Contribution" :value="maxContribution ? `${formatAmount(maxContribution, purchaseTokenDecimals)} ${purchaseTokenSymbol}` : 'No limit'" />
                  <InfoCard label="Max Participants" :value="maxParticipants ? maxParticipants.toString() : 'Unlimited'" />
                  <InfoCard label="Total Sale Amount" :value="formatTokenAmount(totalSaleAmount)" />
                  <InfoCard label="Soft Cap" :value="`${formatAmount(softCap, purchaseTokenDecimals)} ${purchaseTokenSymbol}`" />
                  <InfoCard label="Hard Cap" :value="`${formatAmount(hardCap, purchaseTokenDecimals)} ${purchaseTokenSymbol}`" />
                </div>
              </div>
              ...
              <div v-if="dexPlatforms.length > 0 && isSaleStarted">
                ...
                <div class="bg-gradient-to-r from-[#eacf6f]/10 to-[#f5e590]/10 rounded-lg p-4 mb-3 border border-[#d8a735]/20">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-xs text-gray-600 dark:text-gray-400">Total for Liquidity</p>
                      <p class="text-xl font-bold text-gray-900 dark:text-white">
                        {{ formatAmount(totalRaisedForDex, purchaseTokenDecimals) }} {{ purchaseTokenSymbol }}
                      </p>
                      <p class="text-xs text-[#b27c10] mt-1">{{ totalDexAllocationPercent }}% of Raised Funds</p>
                    </div>
                    <div class="text-right">
                      <p class="text-xs text-gray-600 dark:text-gray-400">Est. Listing</p>
                      <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ estimatedListingTime }}</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="space-y-2">
                  <div
                    v-for="(platform, index) in dexPlatforms"
                    :key="index"
                    class="bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-700"
                  >
                    <div class="flex items-center justify-between">
                      <div class="flex items-center space-x-2">
                        <div class="w-8 h-8 bg-gradient-to-r from-[#b27c10] to-[#e1b74c] rounded-lg flex items-center justify-center">
                          <span class="text-white text-sm font-bold">{{ platform.name.charAt(0) }}</span>
                        </div>
                        <div>
                          <p class="text-sm font-bold text-gray-900 dark:text-white">{{ platform.name }}</p>
                          <p class="text-xs text-[#d8a735]">{{ platform.allocationPercentage }}% Allocation</p>
                        </div>
                      </div>
                      <div class="text-right">
                        <div class="flex items-baseline gap-1 justify-end">
                          <p class="text-sm font-semibold text-gray-900 dark:text-white">
                            {{ formatAmount(BigInt(platform.calculatedPurchaseLiquidity), purchaseTokenDecimals) }} {{ purchaseTokenSymbol }}
                          </p>
              ...
              <div v-else-if="dexPlatforms.length > 0 && !isSaleStarted">
                ...
                <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700 text-center">
                  ...
                  <p class="text-sm font-medium text-gray-900 dark:text-white mb-1">Waiting for Sale Start</p>
              ...
              <!-- Pipeline Timeline (moved to bottom) -->
              <div>
                <h3 class="text-base font-bold text-gray-900 dark:text-white mb-3 flex items-center">
                ...
                <PipelineTimeline
                  :sale-started="isSaleStarted"
                  :softcap-reached="softCapProgress >= 100"
                  :token-deployed="!!deployedContracts.tokenCanister"
                  :distribution-deployed="deployedContracts.vestingContracts.length > 0"
                  :dex-listed="!!deployedContracts.liquidityPool"
            ...
            <div v-show="activeTab === 'tokenomics'" class="space-y-4">
              <div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-3">Token Distribution</h3>
                <DistributionChart
                  v-if="tokenDistributionItems.length > 0"
              ...
              <!-- Vesting Schedules -->
              <div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-3">Vesting Schedules</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <VestingCard
                    v-for="(vesting, index) in vestingSchedules"
                    :key="index"
                    :title="vesting.name"
                    :immediate-release="vesting.immediateRelease"
                    :cliff-days="vesting.cliffDays"
                    :duration-days="vesting.durationDays"
            ...
            <div v-show="activeTab === 'raised-funds'" class="space-y-4">
              <div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-3">Raised Funds Allocation</h3>
                <DistributionChart
                  v-if="raisedFundsItems.length > 0"
                  :items="raisedFundsItems"
              ...
              <div v-if="raisedFundsRecipients.length > 0">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-3">Recipients & Vesting</h3>
                <div class="space-y-3">
                  <div
                    v-for="(recipient, index) in raisedFundsRecipients"
                    :key="index"
                    class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700"
                  >
                    <div class="flex items-start justify-between mb-3">
                      <div>
                        <h4 class="text-base font-semibold text-gray-900 dark:text-white">{{ recipient.category }}</h4>
                        <p v-if="recipient.name" class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ recipient.name }}</p>
                      </div>
                      <div class="text-right">
                        <p class="text-xs text-gray-600 dark:text-gray-400">Allocation</p>
                        <p class="text-base font-bold text-[#d8a735]">{{ recipient.percentage }}%</p>
                      </div>
                    </div>
                    <div v-if="recipient.vestingSchedule" class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                      <div>
                        <p class="text-xs text-gray-600 dark:text-gray-400">Immediate</p>
                        <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ recipient.vestingSchedule.immediateRelease }}%</p>
                      </div>
                      <div>
                        <p class="text-xs text-gray-600 dark:text-gray-400">Cliff</p>
                        <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ recipient.vestingSchedule.cliffDays }}d</p>
                      </div>
                      <div>
                        <p class="text-xs text-gray-600 dark:text-gray-400">Duration</p>
                        <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ recipient.vestingSchedule.durationDays }}d</p>
                      </div>
                      <div>
                        <p class="text-xs text-gray-600 dark:text-gray-400">Frequency</p>
                        <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ recipient.vestingSchedule.releaseFrequency }}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div v-show="activeTab === 'manager'" class="space-y-6">
              ...
              <div class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-6 border border-blue-200 dark:border-blue-700">
                <div class="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400">Check for updates and upgrade your launchpad contract</p>
                  </div>
                  <button
                    @click="checkVersion"
                ...
                <div v-if="versionInfo" class="grid grid-cols-2 gap-4 mt-4">
                  <div class="bg-white dark:bg-gray-800 rounded-lg p-4">
                    <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">Current Version</p>
                    <p class="text-xl font-bold text-gray-900 dark:text-white">{{ versionInfo.current || 'v1.0.0' }}</p>
                  </div>
                  <div class="bg-white dark:bg-gray-800 rounded-lg p-4">
                    <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">Latest Version</p>
                    <p class="text-xl font-bold text-green-600 dark:text-green-400">{{ versionInfo.latest || 'v1.0.0' }}</p>
                  </div>
                </div>

                <div v-if="versionInfo && versionInfo.hasUpdate" class="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
                  <p class="text-sm text-yellow-800 dark:text-yellow-200 mb-3">
                    <span class="font-semibold">New version available!</span> Upgrade to get the latest features and improvements.
                  </p>
                  <button
                    @click="upgradeContract"
                    :disabled="isUpgrading"
                    class="px-4 py-2 bg-yellow-600 hover:bg-yellow-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-sm font-medium transition-colors"
                  >
                ...
                <div class="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows="4"
                      class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#d8a735] focus:border-[#d8a735] dark:bg-gray-700 dark:text-white"
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Discord</label>
                      <input
                    ...
                    <div>
                      <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Medium</label>
                      <input
                    ...
                    <div>
                      <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Reddit</label>
                      <input
                        v-model="editForm.reddit"
                        type="url"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#d8a735] focus:border-[#d8a735] dark:bg-gray-700 dark:text-white"
              ...
              <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                ...
                <div class="space-y-4">
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Project Logo URL</label>
                      <URLImageInput
          ...
          <!-- Sidebar (Action Card & Token Info) -->
          <div class="space-y-4">
            <!-- Timeline Card -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700">
              ...
              <div class="space-y-3">
                <TimelineItem
                  label="Sale Start"
                  :date="formatTimestamp(saleStart)"
                  :status="saleStartStatus"
                  :active="isSaleActive"
                />
                <TimelineItem
                  label="Sale End"
                  :date="formatTimestamp(saleEnd)"
                  :status="saleEndStatus"
                  :active="false"
                />
                <TimelineItem
                  v-if="claimStart"
                  label="Claim Start"
                  :date="formatTimestamp(claimStart)"
                ...
                <TimelineItem
                  v-if="listingTime"
                  label="Listing Time"
                  :date="formatTimestamp(listingTime)"
                  :status="listingTimeStatus"
            ...
            <!-- Action Card -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 sticky top-8">
              <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Participate</h3>

              <!-- ICTO Passport Score Requirement (if required) -->
              <div v-if="minPassportScore > 0" class="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <div class="flex items-center justify-between mb-2">
                  <span class="text-xs font-semibold text-blue-700 dark:text-blue-400">ICTO Passport Required</span>
                  <span class="text-xs font-bold text-blue-900 dark:text-blue-300">{{ minPassportScore }}+ Score</span>
                </div>
                <div class="flex items-center justify-between">
                ...
                <p v-if="userPassportScore < minPassportScore" class="mt-2 text-xs text-red-600 dark:text-red-400">
              ...
              <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 space-y-3">
                ...
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-sm font-semibold text-gray-900 dark:text-white">0 {{ saleTokenSymbol }}</span>
                </div>
              </div>

              <!-- Contract Info -->
              <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                <h4 class="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">Contract</h4>
                <div class="space-y-2">
                  <!-- Canister ID -->
                  <div class="flex items-center space-x-2 bg-gray-50 dark:bg-gray-900 rounded-lg p-2">
                    <code class="text-xs text-gray-600 dark:text-gray-400 flex-1 truncate">{{ canisterIdShort }}</code>
                    <button
                      @click="copyCanisterId"
                      class="text-gray-500 hover:text-[#d8a735] transition-colors"
                  ...
                  <!-- Version -->
                  <div v-if="contractVersion" class="flex items-center justify-between bg-gray-50 dark:bg-gray-900 rounded-lg p-2">
                    <span class="text-xs text-gray-500 dark:text-gray-400">Version:</span>
                    <span class="text-xs font-mono text-gray-700 dark:text-gray-300">{{ contractVersion }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700">
              ...
              <div class="space-y-3">
                <div>
                  ...
                  <p class="text-base font-semibold text-gray-900 dark:text-white">{{ saleTokenName }}</p>
                </div>
                <div>
                  <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Symbol</p>
                  <p class="text-base font-semibold text-gray-900 dark:text-white">{{ saleTokenSymbol }}</p>
                </div>
                <div>
                  <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Total Supply</p>
                  <p class="text-base font-semibold text-gray-900 dark:text-white">{{ formatTokenAmount(Number(totalSupply)) }}</p>
                </div>
                <div>
                  <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Standard</p>
                  <p class="text-base font-semibold text-gray-900 dark:text-white">{{ saleTokenStandard }}</p>
                </div>
              </div>
            </div>
          </div>
...
const isOwner = computed(() => {
  ...
  try {
    ...
    else if (authStore.pnp?.owner) {
      userPrincipal = authStore.pnp.owner.toString()
    }

    console.log('[LaunchpadDetail] Principal check:', {
      fromAuthStore: authStore.principal,
      fromPNP: authStore.pnp?.owner?.toString(),
...
const deployedContracts = computed(() => {
  return launchpad.value?.deployedContracts || {
    tokenCanister: null,
    vestingContracts: [],
    liquidityPool: null,
...
const raisedFundsItems = computed(() => {
  ...
  return allocations
    .filter(alloc => Number(alloc.amount) > 0) // Filter out zero amounts
    .map((alloc, index) => ({
      name: alloc.name,
      value: Number(alloc.amount),
...
const dexPlatforms = computed(() => {
  ...
  return launchpad.value.config.multiDexConfig[0].platforms.map(platform => ({
    ...
    calculatedPurchaseLiquidity: platform.calculatedPurchaseLiquidity
  }))
})
...
// Calculate total raised funds allocated for DEX
const totalRaisedForDex = computed(() => {
  if (!launchpad.value?.config.raisedFundsAllocation) return BigInt(0)
  ...
  return dexAllocation ? BigInt(dexAllocation.amount) : BigInt(0)
})
...
const calculateDexPercentage = (lpValue: any): string => {
  if (!totalRaisedForDex.value || totalRaisedForDex.value === BigInt(0)) return '0.0'

  const lpBigInt = BigInt(lpValue)
  const total = totalRaisedForDex.value
...
const checkVersion = async () => {
  try {
    // TODO: Implement version check via factory
    toast.info('Version check feature coming soon!')

    // Mock version info for now
    versionInfo.value = {
      current: 'v1.0.0',
      latest: 'v1.0.0',
      hasUpdate: false
...
const upgradeContract = async () => {
  ...
  try {
    ...
    // TODO: Implement upgrade via factory
    toast.info('Upgrade feature coming soon!')

    await new Promise(resolve => setTimeout(resolve, 2000))
    toast.success('Contract upgraded successfully!')

    // Reload launchpad data
    await fetchData()
  } catch (err) {
    console.error('Failed to upgrade:', err)
    toast.error('Failed to upgrade contract')
  } finally {
...
const resetEditForm = () => {
  ...
  const info = launchpad.value.config.projectInfo
  editForm.value = {
    description: info.description || '',
    website: info.website?.[0] || '',
    twitter: info.twitter?.[0] || '',
    telegram: info.telegram?.[0] || '',
    discord: info.discord?.[0] || '',
    github: info.github?.[0] || '',
    medium: info.medium?.[0] || '',
    reddit: info.reddit?.[0] || '',
    youtube: info.youtube?.[0] || '',
    logo: projectImages.value.logo || '',  // Use fetched images
    cover: projectImages.value.cover || ''  // Use fetched images
  }
}
...
const saveProjectInfo = async () => {
  ...
  try {
    ...
    // Create update request (only text fields)
    ...
    const updateRequest = {
      description: editForm.value.description ? [editForm.value.description] : [],
      website: editForm.value.website ? [editForm.value.website] : [],
      telegram: editForm.value.telegram ? [editForm.value.telegram] : [],
      twitter: editForm.value.twitter ? [editForm.value.twitter] : [],
      discord: editForm.value.discord ? [editForm.value.discord] : [],
      github: editForm.value.github ? [editForm.value.github] : [],
      medium: editForm.value.medium ? [editForm.value.medium] : [],
      reddit: editForm.value.reddit ? [editForm.value.reddit] : [],
      youtube: editForm.value.youtube ? [editForm.value.youtube] : [],
      whitepaper: [],
      documentation: [],
      auditReport: [],
      kycProvider: [],
    ...
    const result = await actor.updateProjectInfo(updateRequest)
    ...
    if ('ok' in result) {
      ...
      toast.error('err' in result ? result.err : 'Failed to update project information')
    }
  } catch (err) {
...
const saveProjectImages = async () => {
  ...
  try {
    ...
    const imagesUpdate = {
      logo: editForm.value.logo ? [editForm.value.logo] as [string] | [] : [] as [string] | [],
      cover: editForm.value.cover ? [editForm.value.cover] as [string] | [] : [] as [string] | []
    }

    const result = await actor.updateProjectImages(imagesUpdate)

    if ('ok' in result) {
      toast.success('Project images updated successfully!')
      // Reload launchpad data to refresh images
      await fetchData()
    } else {
      toast.error('err' in result ? result.err : 'Failed to update images')
    }
  } catch (err) {
...
const fetchData = async () => {
  ...
  try {
    ...
    // Debug logging for sale parameters
    console.log('📊 Launchpad Data Debug:', {
      rawValues: {
        softCap: data.config.saleParams.softCap.toString(),
        hardCap: data.config.saleParams.hardCap.toString(),
        minContribution: data.config.saleParams.minContribution.toString(),
        maxContribution: data.config.saleParams.maxContribution?.[0]?.toString() || 'null'
      },
      timeline: {
        saleStart: data.config.timeline.saleStart.toString(),
        saleEnd: data.config.timeline.saleEnd.toString()
      },
      deployedContracts: {
        tokenCanister: data.deployedContracts.tokenCanister?.[0] || null,
        vestingContracts: data.deployedContracts.vestingContracts.length,
        liquidityPool: data.deployedContracts.liquidityPool?.[0] || null
      },
      decimals: {
        purchase: data.config.purchaseToken.decimals,
        sale: data.config.saleToken.decimals
      }
    })
...
const copyCanisterId = async () => {
  try {
    await navigator.clipboard.writeText(canisterId.value)
...
const handleDeposit = () => {
  ...
  // Open deposit modal
  console.log('🎯 Opening deposit modal with data:', {
    canisterId: canisterId.value,
