...
persistent actor MultisigFactory {
    ...
    private func _notifyContractCompleted(contractId: Principal, newVersion: VersionManager.Version) : async () {
        try {
            ...
            let result = await contractActor.completeUpgrade(newVersion, Principal.fromActor(MultisigFactory));
    ...
    private func _notifyContractCancelled(contractId: Principal, reason: Text) : async () {
        try {
            ...
            let result = await contractActor.cancelUpgrade(reason, Principal.fromActor(MultisigFactory));
