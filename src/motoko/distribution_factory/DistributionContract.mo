...
persistent actor class DistributionContract(initArgs : DistributionUpgradeTypes.DistributionInitArgs) = self {
  ...
  // Registration lock to prevent concurrent registration race conditions
  // Maps Principal -> true if registration is in progress for that user
  private transient var registrationLocks : Trie.Trie<Principal, Bool> = Trie.empty();

  // Category participant counters (cached for performance)
  // Maps CategoryId -> participant count for that category
  private transient var categoryParticipantCounts : Trie.Trie<Nat, Nat> = Trie.empty();
  ...
  private func _reentrancyGuardEnter(caller : Principal) : Result.Result<(), Text> {
    ...
    let existingGuard = Array.find(
      _userReentrancyGuards,
      func((user, _) : (Principal, Bool)) : Bool {
    ...
    switch (existingGuard) {
      case (?(_, isLocked)) {
        ...
        let filteredGuards = Array.filter(
          _userReentrancyGuards,
          func((user, _) : (Principal, Bool)) : Bool {
  ...
  private func _hasRole(caller : Principal, requiredRole : Role) : Bool {
    ...
    let userRole = Array.find(
      _adminRoles,
      func((user, _role) : (Principal, Role)) : Bool {
        Principal.equal(user, caller);
      },
    );
  ...
  private func _validateAmount(amount : Nat) : Result.Result<(), Text> {
    ...
    if (amount > 1_000_000_000_000_000_000) {
      // 1 billion tokens (adjust as needed)
  ...
  /// Check if a registration is currently in progress for a user
  /// Returns true if locked, false if available
  private func _isRegistrationLocked(principal : Principal) : Bool {
    switch (Trie.get(registrationLocks, _principalKey(principal), Principal.equal)) {
      case (?locked) { locked };
      case null { false };
    };
  };

  /// Acquire registration lock for a user
  /// Returns true if lock acquired successfully, false if already locked
  private func _acquireRegistrationLock(principal : Principal) : Bool {
    if (_isRegistrationLocked(principal)) {
      return false;
    };
    registrationLocks := Trie.put(registrationLocks, _principalKey(principal), Principal.equal, true).0;
    true;
  };

  /// Release registration lock for a user
  private func _releaseRegistrationLock(principal : Principal) {
    registrationLocks := Trie.put(registrationLocks, _principalKey(principal), Principal.equal, false).0;
  };

  /// Get participant count for a specific category (cached)
  private func _getCategoryParticipantCount(categoryId : Nat) : Nat {
    switch (Trie.get(categoryParticipantCounts, _natKey(categoryId), Nat.equal)) {
      case (?count) { count };
      case null {
        // Calculate count by iterating participants
        var count = 0;
        ...
        // Cache the count
        categoryParticipantCounts := Trie.put(categoryParticipantCounts, _natKey(categoryId), Nat.equal, count).0;
        count;
      };
    };
  };
  ...
  private func _incrementCategoryParticipantCount(categoryId : Nat) {
    let currentCount = _getCategoryParticipantCount(categoryId);
    categoryParticipantCounts := Trie.put(categoryParticipantCounts, _natKey(categoryId), Nat.equal, currentCount + 1).0;
  };
  ...
  private func _isActive() : Bool {
    status == #Active and Time.now() >= config.distributionStart and (Option.isNull(config.distributionEnd) or Time.now() <= Option.get(config.distributionEnd, Time.now()))
  };
  ...
  public shared ({ caller }) func claimWithMerkleProof(
    ...
    let root = switch (rootOpt) {
      case (?r) { r.1 };
      case null {
        return #err("No merkle root for category " # Nat.toText(categoryId));
      };
    };
    ...
    switch (transferResult) {
      ...
      case (#err(err)) {
        ...
        let errorText = switch (err) {
          case (#BadBurn(badBurn)) {
            "Bad burn: minimum burn amount is " # Nat.toText(badBurn.min_burn_amount);
          };
          case (#BadFee(badFee)) {
            "Bad fee: expected fee is " # Nat.toText(badFee.expected_fee);
          };
          case (#InsufficientFunds(insufficient)) {
            "Insufficient funds: available balance is " # Nat.toText(insufficient.balance);
          };
          case (#TooOld) { "Transaction too old" };
          case (#CreatedInFuture(created)) {
            "Transaction created in future: " # Nat64.toText(created.ledger_time);
          };
          case (#Duplicate(duplicate)) {
            "Duplicate transaction of " # Nat.toText(duplicate.duplicate_of);
          };
          case (#TemporarilyUnavailable) { "Service temporarily unavailable" };
          case (#GenericError(error)) {
            Nat.toText(error.error_code) # ": " # error.message;
          };
        };
  ...
  private func detectSybilBehavior(caller : Principal) : async SybilRiskScore {
    ...
    if (participant.totalEligible > 1_000_000) {
      // > 1M tokens
      score += 10;
    ...
    let categoriesWithoutPassport = Array.filter<CategoryAllocation>(
  ...
  public query func detectPotentialDuplicates() : async [{
    ...
    for (participant in allParticipants.vals()) {
      ...
      if (score > 0) {
        suspiciousParticipants := Array.append(suspiciousParticipants, [{ principal = participant.principal; riskScore = score; reasons = reasons }]);
      };
    };

    // Sort by risk score (highest first)
    Array.sort<{ principal : Principal; riskScore : Nat; reasons : [Text] }>(
      suspiciousParticipants,
      func(a, b) {
        if (a.riskScore > b.riskScore) { #greater } else if (a.riskScore < b.riskScore) {
          #less;
        } else { #equal };
      },
    );
  ...
  public query func getCategoryAnalytics() : async [CategoryAnalytics] {
    ...
    for (participant in allParticipants.vals()) {
      for (category in participant.categories.vals()) {
        let existing = Array.find<(Nat, { totalEligible : Nat; totalClaimed : Nat; activeParticipants : Nat; categoryNames : [Text] })>(categoryMap, func(item) { item.0 == category.categoryId });
        ...
        switch (existing) {
          case (?(_, data)) {
            ...
            categoryMap := Array.map<(Nat, { totalEligible : Nat; totalClaimed : Nat; activeParticipants : Nat; categoryNames : [Text] }), (Nat, { totalEligible : Nat; totalClaimed : Nat; activeParticipants : Nat; categoryNames : [Text] })>(
              categoryMap,
              func(item) {
          ...
          case null {
            // Add new category
            categoryMap := Array.append(categoryMap, [(category.categoryId, { totalEligible = category.amount; totalClaimed = category.claimedAmount; activeParticipants = if (category.claimedAmount > 0) { 1 } else { 0 }; categoryNames = []; /* We'll populate from participant data */ })]);
          };
        };
      };
    };
    ...
    Array.map<(Nat, { totalEligible : Nat; totalClaimed : Nat; activeParticipants : Nat; categoryNames : [Text] }), CategoryAnalytics>(
      categoryMap,
      func(item) {
  ...
  public query func getDistributionHealth() : async DistributionHealth {
    ...
    let activeParticipants = Array.filter<MultiCategoryParticipant>(
      allParticipants,
      func(p) {
        p.totalClaimed > 0;
      },
    ...
    if (hoursElapsed < 24.0 * 7.0 and activeParticipants > 0) {
      healthScore += 10;
    }; // Early activity
  ...
  public query func healthCheck() : async IUpgradeable.HealthStatus {
    ...
    if (cyclesBalance < 100_000_000_000) {
      // Less than 0.1T cycles
  ...
  public shared ({ caller }) func register(categoryId : ?Nat) : async Result.Result<DistributionTypes.BatchRegisterResult, Text> {
    ...
    // RACE CONDITION FIX #1: Acquire registration lock to prevent concurrent registrations
    // This prevents the same user from registering multiple times simultaneously
    if (not _acquireRegistrationLock(caller)) {
      return #err("Registration already in progress. Please wait.");
    };

    // Get existing participant if any
    let existingParticipant = Trie.get(participants, _principalKey(caller), Principal.equal);
          ...
          case (?cats) {
            switch (DistributionTypes.findCategoryById(cats, specificId)) {
              ...
              case null {
                ...
                return #err("Category " # Nat.toText(specificId) # " not found");
              };
            };
          };
          case null {
            _releaseRegistrationLock(caller);
            return #err("No categories configured");
          };
        };
      };
      case null {
        ...
        switch (config.categories) {
          case (?cats) {
            Array.filter<DistributionTypes.DistributionCategory>(
              cats,
              func(cat) {
                switch (cat.mode) {
                  case (#Open) { true };
    ...
    for (category in categoriesToProcess.vals()) {
      ...
      // Check if user already registered for this specific category
      var alreadyInCategory = false;
      switch (existingParticipant) {
        case (?participant) {
          for (existingCat in participant.categories.vals()) {
            if (existingCat.categoryId == category.id) {
      ...
      if (alreadyInCategory) {
        ...
        categoryMessage := "Already registered for this category";
      } else {
        ...
        switch (category.mode) {
          ...
          case (#Open) {
            // Check passport score requirement for Open category
            let requiredScore = category.defaultPassportScore;

            // If passport score = 0, allow all (passport check disabled)
            var hasValidPassport = requiredScore == 0;

            if (not hasValidPassport) {
              // Check passport score based on provider
              switch (category.defaultPassportProvider) {
                case "ICTO" {
                  hasValidPassport := await _checkICTOPassportScore(caller, requiredScore);
                };
                case _ {
                  // For other providers, fail for now
            ...
            if (not hasValidPassport) {
              categorySuccess := false;
              categoryMessage := "Passport score insufficient for this category (required: " # Nat.toText(requiredScore) # ")";
            } else {
              ...
              // RACE CONDITION FIX #2: Check max participants for THIS specific category
              switch (category.maxParticipants) {
                case (?maxParticipants) {
                  // Count actual participants registered in THIS category
                  let currentCategoryCount = _getCategoryParticipantCount(category.id);
                  if (currentCategoryCount >= maxParticipants) {
                    categorySuccess := false;
                    categoryMessage := "Category reached maximum participants (" # Nat.toText(currentCategoryCount) # "/" # Nat.toText(maxParticipants) # ")";
                  };
                };
                case null {};
              };
    ...
    // Create or update participant record if any category registration succeeded
    if (hasAnySuccess and newCategories.size() > 0) {
      switch (existingParticipant) {
        case (?existing) {
          // Update existing participant with new categories
          let allCategories = Array.append<CategoryAllocation>(existing.categories, newCategories);
          let participant : MultiCategoryParticipant = {
            principal = caller;
            registeredAt = existing.registeredAt; // Keep original registration time
            categories = allCategories;
            totalEligible = existing.totalEligible + totalAllocated;
          ...
          // Increment category participant counts for newly added categories
          for (newCat in newCategories.vals()) {
            _incrementCategoryParticipantCount(newCat.categoryId);
          };
          // Don't increment participantCount as user already existed
        };
        case null {
          ...
          // Increment category participant counts for all categories
          for (newCat in newCategories.vals()) {
            _incrementCategoryParticipantCount(newCat.categoryId);
          };
    ...
    // Release registration lock before returning
    _releaseRegistrationLock(caller);
  ...
  public query func getDistributionMode() : async {
    usingMerkle : Bool;
    hasMerkleConfig : Bool;
  } {
    {
  ...
  public shared ({ caller }) func claimFromCategory(categoryId : Nat, claimAmount : ?Nat) : async Result.Result<{ claimedAmount : Nat; categoryId : Nat }, Text> {
    ...
    let category = switch (targetCategory) {
      case (?cat) { cat };
      case null {
        return #err("Category " # Nat.toText(categoryId) # " not found for this participant");
      };
    };
    ...
    let actualClaimAmount = switch (claimAmount) {
      case (?amount) {
        ...
        if (amount > categoryClaimable) {
          return #err("Claim amount exceeds available tokens in category");
        };
        amount;
      };
  ...
  public shared ({ caller }) func claimBatch(categoryId : ?Nat, claimAmount : ?Nat) : async Result.Result<DistributionTypes.BatchClaimResult, Text> {
    ...
    let categoriesToProcess : [CategoryAllocation] = switch (categoryId) {
      ...
      case null {
        ...
        Array.filter<CategoryAllocation>(
          participant.categories,
          func(cat) {
    ...
    for (participantCat in participant.categories.vals()) {
      ...
      if (shouldProcess and categoryClaimable > 0 and remainingClaim > 0) {
        ...
        if (hasValidPassport and categoryClaimAmount > 0) {
          ...
          let errorMsg = if (not hasValidPassport) {
            "Passport verification failed";
          } else { "No claimable amount" };
  ...
  public shared ({ caller }) func earlyUnlockWithPenalty() : async Result.Result<{ unlockedAmount : Nat; penaltyAmount : Nat }, Text> {
    ...
    let penaltyConfig = switch (config.penaltyUnlock) {
      ...
      case null {
        return #err("Early unlock with penalty is not enabled for this distribution");
      };
    };
  ...
  public query func getCategoryBreakdown(principal : Principal) : async [{
    ...
    switch (Trie.get(participants, _principalKey(principal), Principal.equal)) {
      case (?participant) {
        let breakdown = Buffer.Buffer<{ categoryId : Nat; categoryName : Text; allocatedAmount : Nat; claimedAmount : Nat; remainingAmount : Nat; claimableAmount : Nat; unlockedAmount : Nat; vestingSchedule : VestingSchedule; vestingStart : Time.Time; note : ?Text }>(0);
  ...
  public query func getClaimableBreakdown(principal : Principal) : async [{
    ...
    switch (Trie.get(participants, _principalKey(principal), Principal.equal)) {
      case (?participant) {
        let breakdown = Buffer.Buffer<{ categoryId : Nat; categoryName : Text; claimableAmount : Nat; totalUnlocked : Nat; percentageOfTotal : Float }>(0);
  ...
  public query func getVestingProgress(principal : Principal) : async [{
    ...
    switch (Trie.get(participants, _principalKey(principal), Principal.equal)) {
      case (?participant) {
        let progress = Buffer.Buffer<{ categoryId : Nat; categoryName : Text; progressPercentage : Float; isFullyVested : Bool; timeUntilNextUnlock : ?Nat; nextUnlockAmount : ?Nat }>(0);
  ...
  /// Get detailed category information for all categories of a user
  /// Returns complete status including vesting calculation for each category
  public query func getUserCategoryInfo(principal : Principal) : async [{
    categoryId : Nat;
    categoryName : Text;
    allocatedAmount : Nat;
    claimedAmount : Nat;
    ...
    vestingSchedule : VestingSchedule;
    vestingStart : Time.Time;
    vestingProgressPercentage : Float;
    isFullyVested : Bool;
    isFullyClaimed : Bool;
    note : ?Text;
  }] {
    switch (Trie.get(participants, _principalKey(principal), Principal.equal)) {
      case (?participant) {
        let categoriesInfo = Buffer.Buffer<{ categoryId : Nat; categoryName : Text; allocatedAmount : Nat; claimedAmount : Nat; remainingAmount : Nat; claimableAmount : Nat; unlockedAmount : Nat; lockedAmount : Nat; vestingSchedule : VestingSchedule; vestingStart : Time.Time; vestingProgressPercentage : Float; isFullyVested : Bool; isFullyClaimed : Bool; note : ?Text }>(0);
        ...
        for (category in participant.categories.vals()) {
          ...
          let lockedAmount = if (category.amount > unlockedAmount) {
            Nat.sub(category.amount, unlockedAmount);
          } else { 0 };

          let vestingProgressPercentage = if (category.amount > 0) {
            Float.fromInt(unlockedAmount) / Float.fromInt(category.amount) * 100.0;
          } else { 0.0 };

          let isFullyVested = unlockedAmount >= category.amount;
          let isFullyClaimed = category.claimedAmount >= category.amount;
          ...
          categoriesInfo.add({
            ...
            vestingSchedule = category.vestingSchedule;
            vestingStart = category.vestingStart;
            vestingProgressPercentage = vestingProgressPercentage;
            isFullyVested = isFullyVested;
        ...
        Buffer.toArray(categoriesInfo);
      };
      case null { [] }; // Participant not found
    };
  };
  ...
  public shared ({ caller }) func whoamiWithCategories() : async {
    ...
    let categoryBreakdown = switch (participant) {
      case (?p) {
        let breakdown = Buffer.Buffer<{ categoryId : Nat; categoryName : Text; claimableAmount : Nat; allocatedAmount : Nat; claimedAmount : Nat; progressPercentage : Float }>(0);
  ...
  public query func getParticipantsPaginated(
    ...
    let pageData = if (startIndex >= totalItems) { [] } else {
  ...
  public shared ({ caller }) func getAllParticipantsWithLockedTokens() : async [{
    ...
    //Only allow canister-to-canister queries
    assert Principal.isCanister(caller);
    let buffer = Buffer.Buffer<{ principal : Principal; remaining : Nat; remaining_time : Nat; max_lock : Nat; contract_id : Principal }>(0);
  ...
  public shared ({ caller }) func addRole(user : Principal, role : Role) : async Result.Result<(), Text> {
    ...
    let existingRole = Array.find(
      _adminRoles,
      func((u, _) : (Principal, Role)) : Bool {
  ...
  public shared ({ caller }) func removeRole(user : Principal) : async Result.Result<(), Text> {
    ...
    let filteredRoles = Array.filter(
      _adminRoles,
      func((u, _) : (Principal, Role)) : Bool {
  ...
  public shared ({ caller }) func getSystemHealthAsync() : async SystemHealth {
    ...
    let remainingAmount = if (contractBalance >= totalClaimedAmount) {
      Nat.sub(contractBalance, totalClaimedAmount);
    } else { 0 };
